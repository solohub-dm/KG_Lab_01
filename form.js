...
function reViewLine() {
    if(isCorrectCoords) {
        trapez.proper.colorLine = trColorsCbx[1].checked ? trColors[1].value : "transparent";
        if (isCorrectHeight) {
            if (isCorrectBasisLen)
                drawPreview();
            else 
                drawBasisHeight();
        } else {
...
function checkCoords(input) {
    ...
    if (!isSomeCoordEmpty() && !isErrorCoords) {
        if (
            ...
            showErrorMessageCoords("The base of a trapezium cannot be a point.", input);
            isErrorCoords = true;
        } else {
...
function checkCoordInput(input) {
    ...
    if (input.value.trim() === "") {
        ...
        showErrorMessageCoords("Value cannot be greater then " + divsCnt + ".", input);
    } else if (inputValue < -divsCnt) {
        showErrorMessageCoords("Value cannot be lower then " + -divsCnt + ".", input);
    }
}
...
function checkHeight() {
    ...
    do {
        ...
        if (!isCorrectCoords) {
            showErrorMessageLength("Enter all coords of larger bassis first.", trHeightLen);
            break;
        }
        if (trHeightLen.value < eps) {
            showErrorMessageLength("Height lenght cannot be lower than " + roundTwoAfter(eps) + ".", trHeightLen);
            break;
        }
        if (trHeightLen.value > divsCnt * 2) {
            showErrorMessageLength("Height cannot be higher than system of coordinates. Max allowed value: " + divsCnt * 2 + ".", trHeightLen);
            break;
        }
        ...
        if (!sqStartPoint.isInRangeAbs(divsCnt) || !sqEndPoint.isInRangeAbs(divsCnt)) {
            ...
            if (smallerLenMax <= eps) {
                ...
                showErrorMessageLength(
                    ...
                    "Max allowed length: " + roundTwoAfter(maxHeight) + ".", trHeightLen);
                break;
            }
        } else {
...
function calcHightLenMax(p1, p2) {
    ...
    if (intersections.length === 0) 
        lenMax = p1.distanceTo(p2);
    else if (intersections.length === 1) 
        lenMax = p1.distanceTo(intersections[0]);
...
function checkSmaller() {
    ...
    do {
        ...
        if (!isCorrectHeight) {
            if (isSomeCoordEmpty()) {
                showErrorMessageLength("Enter all coords of larger bassis first.", trSmallBasisLen);
        ...
        if (smallerLen < eps) {
            showErrorMessageLength("Smaller basis lenght cannot be lower than " + roundTwoAfter(eps) + ".", trSmallBasisLen);
            break;
        }
        if (smallerLen >= len) {
