...
function changeWindow() {
  if (isLayersMode) windowBtnText.textContent = "Show layers menu";
  else windowBtnText.textContent = "Create trapezium";
...
function deleteAllTrapeziums() {
  ...
  const res = confirm("Are you sure to delete all trapeziums?");
  if (!res) return;
...
function createTrapezium() {
  ...
  if (trName.value.trim() === "")
    ...
    trHeightLen.value.trim() === "" ||
    trSmallBasisLen.value.trim() === ""
  ) {
...
function createTrapeziumDiv(trapez) {
  ...
  colorDisplay.classList.add("trapezium-colors");
  colorDisplay.style.backgroundColor = trapez.proper.colorFill;
  colorDisplay.style.borderColor = trapez.proper.colorLine;
  ...
  newTrapezium.appendChild(colorDisplay);
...
function showTrapezium(trapeziumElement, showButton) {
  const index = trapeziums.findIndex(
    (item) => item.divElm === trapeziumElement
  );
  // Зміна зображення
  if (showButton.alt === "hide") {
    clearTrapezuim(index);
    trapeziums[index].isShow = false;
    showButton.alt = "show";
    showButton.src = "./img/hide.png"; // Заміна на інше зображення
  } else {
    trapeziums[index].isShow = true;
...
function clearTrapezuim(index) {
  ...
  let luPC = new Point(cX(luP.x) - offs, cX(-luP.y - offs));
  let offset = new Point(
    cX(rdP.x) - luPC.x + offs * 2,
    cX(-rdP.y) - luPC.y + offs * 2
  );

  ctx.clearRect(luPC.x, luPC.y, offset.x, offset.y);
  ctx.drawImage(
    canvasSave,
    luPC.x,
    luPC.y,
  ...
  for (let i = 0; i < trapeziums.length; i++) {
    if (i == index) continue;
    if (tryTrapeziumsOverlap(trapeziums[i], rect)) draw(trapeziums[i]);
  }
}
...
function deleteTrapezium(trapeziumElement) {
  ...
  if (!trapeziums.length) return;
  const res = confirm(
    'Are you sure to delete trapezium "' + trapeziums[index].proper.name + '"?'
  );
  if (!res) return;
...
function reDrawTrapezium(index) {
  ...
  for (let i = index + 1; i < trapeziums.length; i++) {
    if (tryTrapeziumsOverlap(trapeziums[i], trapeziums[index])) {
      draw(trapeziums[i]);
    }
  }
}
...
function moveTrapeziumUp(trapeziumElement) {
  ...
  parent.insertBefore(
    trapeziumElement,
    trapeziumElement.previousElementSibling
  );
  // Оновити масив
  ...
  if (index < trapeziums.length - 1) {
    [trapeziums[index], trapeziums[index + 1]] = [
      trapeziums[index + 1],
      trapeziums[index],
    ];

    if (tryTrapeziumsOverlap(trapeziums[index], trapeziums[index + 1])) {
...
function tryTrapeziumsOverlap(trap1, trap2) {
  ...
  if (trap2.proper) if (!trap2.isShow || !trap2.inSyst) return false;
...
function trapeziumsOverlap(trap1, trap2) {
  ...
  if (
    pTr1.some((p) => isPointInsideTrapezium(p, pTr2)) ||
